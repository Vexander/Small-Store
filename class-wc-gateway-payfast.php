 ...
 *
 * Provides a PayFast Payment Gateway.  Updated by Red Lorry FOr My Small Store.
 *
 ...
 * @author		WooThemes
 */
define( 'WC_GATEWAY_PAYFAST_VERSION', '1.4.4' );
...
class WC_Gateway_PayFast extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			...
			if ( !empty($vendor_form_data) ) {
				$this->merchant_id 		= $vendor_form_data[1][value];
				$this->merchant_key 	= $vendor_form_data[2][value];
				$this->pass_phrase  	= $vendor_form_data[3][value];
			} 
			else {
	...
	public function admin_options() {
		...
		?>
			...
			<div class="inline error"><p><strong><?php _e( 'Gateway Disabled', 'woocommerce-gateway-payfast' ); ?></strong> <?php /* translators: 1: a href link 2: closing href */ echo sprintf( __( 'Choose South African Rands as your store currency in %1$sPricing Options%2$s to enable the PayFast Gateway.', 'woocommerce-gateway-payfast' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=general' ) ) . '">', '</a>' ); ?></p></div>
			<?php
		}
	}
